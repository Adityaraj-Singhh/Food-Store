...
const UpdateProduct = ({product}: {product : ProductType}) => {
	...
	const handleUpdateValue = (e: SyntheticEvent) => {
		...
		if (target.name === 'img' && imageTag.current && target.type === 'file') {
			const fileInput = target as HTMLInputElement;
			if (fileInput.files) {
				imageIsChanged = true;
				imageTag.current.src = URL.createObjectURL(fileInput.files[0]);
			}
		}

		const value = target.type === 'checkbox' ? (target as HTMLInputElement).checked : target.value;
		setUpdateData(prevState => ({ ...prevState, [target.name]: value }));
		...
		<form action="" method="patch" className="checkout-service p-3" onSubmit={handleSubmit}>
			...
			<div className='mt-4'>
					<label className='w-100'>
						<span>Category</span>
						<select name="categorie_id" className='form-select w-100' value={updateData.categorie_id || ''} onChange={handleUpdateValue}>
			...
			<div>
				<label>
					<input type="checkbox" name="deal_of_day" checked={(updateData as any).deal_of_day || false} onChange={handleUpdateValue} />
